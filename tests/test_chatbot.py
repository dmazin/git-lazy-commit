...
class TestChatBot(unittest.TestCase):
    ...
    def test_init_with_system_message(self, mock_create):
        ...
        self.assertEqual(chatbot.messages, [{"role": "system", "content": self.system_message}])
    ...
    def test_call(self, mock_create):
        ...
        mock_create.return_value = Mock(choices=[Mock(message=Mock(content=chatbot_response))])

        response = chatbot(user_message)

        self.assertEqual(response, chatbot_response)
        self.assertEqual(chatbot.messages[-1], {"role": "assistant", "content": chatbot_response})
